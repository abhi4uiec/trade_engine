	...
	if n != 0 || book.Asks[n-1].Price.LessThanOrEqual(order.Price) {
		...
		for i := n - 1; i >= 0; i-- {
			...
			if sellOrder.Quantity.GreaterThanOrEqual(order.Quantity) {
				...
				if len(book.Bids) > 0 {
					book.removeBuyOrder(order.ID)
				}
				// if sellOrder.Quantity = 0
				if sellOrder.Quantity.IsZero() {
					book.removeSellOrder(sellOrder.ID)
				}
				return trades
			}
...
func (book *OrderBook) processLimitSellOrder(order Order) []Trade {
	...
	if n != 0 || book.Bids[n-1].Price.GreaterThanOrEqual(order.Price) {
		...
		for i := n - 1; i >= 0; i-- {
			...
			//if buyOrder.Price.GreaterThanOrEqual(order.Price) { Abhishek changed
			if buyOrder.Quantity.GreaterThanOrEqual(order.Quantity) {
				trades = append(
					trades,
					Trade{
						TakerOrderID: order.ID,
				...
				if len(book.Asks) > 0 {
					book.removeSellOrder(order.ID)
				}
				// if buyOrder.Quantity = 0
				if buyOrder.Quantity.IsZero() {
					book.removeBuyOrder(buyOrder.ID)
				}
				return trades
			}
